...
class StateOptionsTrait
{
    ...
    public function getStateOptionsClass(Operation $operation, ?string $defaultClass = null, string $optionsType = OptionsInterface::class): ?string
    {
        ...
        if (class_exists(Options::class) && $options instanceof Options && $options->getEntityClass()) {
            return $options->getEntityClass() ?? $defaultClass;
        }

        if (class_exists(ODMOptions::class) && $options instanceof ODMOptions && $options->getDocumentClass()) {
            return $options->getDocumentClass() ?? $defaultClass;
        }

        if (class_exists(EloquentOptions::class) && $options instanceof EloquentOptions && $options->getModelClass()) {
            return $options->getModelClass() ?? $defaultClass;
        }
