...
class ExtractorResourceMetadataFactoryTest extends FileConfigurationMetadataFactoryProvider
{
    ...
    {
        $this->expectException(InvalidArgumentException::class);
        $this->{method_exists($this, 'expectExceptionMessageMatches') ? 'expectExceptionMessageMatches' : 'expectExceptionMessageRegExp'}('/"ApiPlatform\\\\Core\\\\Tests\\\\Fixtures\\\\TestBundle\\\\Entity\\\\Dummy" setting is expected to be null or an array, string given in ".+\\/Fixtures\\/FileConfigurations\\/bad_declaration\\.yml"\\./');
